...
for (const botaoEl of botoes) {
  botaoEl.addEventListener('click', alteraImagem);
}

function alteraImagem(event) {
  const acao = event.currentTarget.innerHTML;
  ...
  imagemSlideEl.src = servidorDasImagens + '/' + imagens[index].arquivo;
  imagemSlideEl.alt = imagens[index].descricao;
}
